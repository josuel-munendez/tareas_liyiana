...
// Implementación de sin() usando series de Taylor
...
// Implementación de sin() usando aproximación polinómica (más eficiente)
function mi_sin_polinomico($x) {
    // Reducir el ángulo al rango [0, 2π]
    $x = fmod($x, 2 * M_PI);
    if ($x < 0) $x += 2 * M_PI;

    // Aproximación polinómica de Bhaskara (siglo VII)
    // sin(x) ≈ (16x(π-x)) / (5π² - 4x(π-x))
    if ($x >= 0 && $x <= M_PI) {
        return (16 * $x * (M_PI - $x)) / (5 * M_PI * M_PI - 4 * $x * (M_PI - $x));
    } else {
        return -mi_sin_polinomico($x - M_PI);
    }
}

// Implementación de sin() usando lookup table con interpolación
function mi_sin_lookup($x, $precision = 1000) {
    // Reducir el ángulo al rango [0, 2π]
    $x = fmod($x, 2 * M_PI);
    if ($x < 0) $x += 2 * M_PI;

    // Crear tabla de lookup si no existe
    static $sin_table = null;
    if ($sin_table === null) {
        $sin_table = array();
        for ($i = 0; $i <= $precision; $i++) {
            $angle = ($i / $precision) * 2 * M_PI;
            $sin_table[$i] = sin($angle); // Usamos nativo para la tabla
        }
    }
    ...
    if ($index_floor == $index_ceil) {
        return $sin_table[$index_floor];
    }

    $weight = $index - $index_floor;
    return $sin_table[$index_floor] * (1 - $weight) + $sin_table[$index_ceil] * $weight;
}

// Función principal que elige el método
function mi_sin($x, $metodo = 'auto', $terminos_taylor = 10) {
    switch ($metodo) {
        case 'taylor':
            return mi_sin_taylor($x, $terminos_taylor);
        case 'polinomico':
            return mi_sin_polinomico($x);
        case 'lookup':
            return mi_sin_lookup($x);
        case 'auto':
        default:
            // Para alta precisión usar Taylor, para eficiencia usar polinómico
            return mi_sin_taylor($x, $terminos_taylor);
    }
}
...
// Función para comparar con sin() nativo de PHP
...
function mostrarProcesoTaylor($x, $terminos = 5) {
    $proceso = "Serie de Taylor para sin(x):\n";
    $proceso .= "sin(x) = ";
    ...
    for ($n = 0; $n < $terminos; $n++) {
        ...
        $termino = pow($x, $exponente) / $factorial;
    ...
    $resultado_parcial = 0;
    for ($n = 0; $n < $terminos; $n++) {
        $termino_valor = pow(-1, $n) * pow($x, 2 * $n + 1) / factorial(2 * $n + 1);
...
function ejecutarEnConsola() {
    echo "=================================\n";
    echo "    IMPLEMENTACIÓN DE SIN()\n";
    echo "   Cálculo del Seno\n";
    echo "          (Modo Consola)\n";
    echo "=================================\n\n";

    echo "Métodos disponibles:\n";
    echo "- Serie de Taylor: sin(x) = x - x³/3! + x⁵/5! - x⁷/7! + ...\n";
    echo "- Aproximación polinómica (Bhaskara)\n";
    ...
    $resultado_taylor = mi_sin($angulo_rad, 'taylor', 10);
    $resultado_polinomico = mi_sin($angulo_rad, 'polinomico');
    $resultado_lookup = mi_sin($angulo_rad, 'lookup');
    $resultado_auto = mi_sin($angulo_rad, 'auto');
    $nativo = sin($angulo_rad);
    ...
    foreach ($angulos_importantes as $i => $angulo_val) {
        $mi_calc = mi_sin($angulo_val, 'taylor', 10);
        $nativo_val = sin($angulo_val);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['angulo'])) {
            ...
            if ($angulo === false) {
                ...
                // Calcular con diferentes métodos
                $resultados['taylor'] = mi_sin($angulo_rad, 'taylor', 10);
                $resultados['polinomico'] = mi_sin($angulo_rad, 'polinomico');
                $resultados['lookup'] = mi_sin($angulo_rad, 'lookup');
                $resultados['auto'] = mi_sin($angulo_rad, 'auto');
                $resultados['nativo'] = sin($angulo_rad);
                ...
                <div class='detalles-calculo'>
                    ...
                    <div class='resultados-comparacion'>
                        ...
                        <div class='resultado-item nativo'>
                            <span class='etiqueta'>🏁 PHP sin() nativo:</span>
...
<head>
    ...
    <title>Implementación de sin() - Web</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>🧮 Implementación de sin()</h1>
        <div class="subtitle">Cálculo del seno con diferentes métodos numéricos</div>
        ...
        <div class="metodos">
            ...
            <div class="metodo">
                ...
                <p>Fórmula de Bhaskara (siglo VII)</p>
                <p><strong>Precisión:</strong> Media</p>
                <p><strong>Eficiencia:</strong> Alta</p>
            </div>
            <div class="metodo">
                <h4>📋 Tabla de Búsqueda</h4>
                <p>Interpolación lineal en tabla precalculada</p>
                <p><strong>Precisión:</strong> Configurable</p>
                <p><strong>Eficiencia:</strong> Muy alta</p>
            </div>
        </div>
        ...
        <div class="info">
            ...
            <ul>
                <li><strong>Serie de Taylor:</strong> Precisión matemática pero requiere más cálculos</li>
                <li><strong>Aproximación de Bhaskara:</strong> Fórmula histórica del siglo VII, sorprendentemente precisa</li>
                <li><strong>Tabla de búsqueda:</strong> Método usado en hardware antiguo, muy rápido</li>
                <li>Puede ingresar ángulos en grados añadiendo 'g' o 'grados' al final</li>
                <li>El seno es periódico con período 2π (360°)</li>
            </ul>
        </div>
    </div>
...
function generarTablaValoresImportantes() {
    $angulos = array(
        array("valor" => 0, "nombre" => "0", "grados" => 0),
        array("valor" => M_PI/6, "nombre" => "π/6", "grados" => 30),
        array("valor" => M_PI/4, "nombre" => "π/4", "grados" => 45),
        array("valor" => M_PI/3, "nombre" => "π/3", "grados" => 60),
        array("valor" => M_PI/2, "nombre" => "π/2", "grados" => 90),
        array("valor" => M_PI, "nombre" => "π", "grados" => 180),
        array("valor" => 3*M_PI/2, "nombre" => "3π/2", "grados" => 270),
        array("valor" => 2*M_PI, "nombre" => "2π", "grados" => 360)
    );

    $html = '<h3>📋 Valores Importantes del Seno</h3>';
    $html .= '<table class="tabla-valores">';
    $html .= '<tr><th>Ángulo</th><th>Radianes</th><th>Grados</th><th>sin(x) Exacto</th><th>sin(x) Taylor</th><th>Error</th></tr>';

    foreach ($angulos as $angulo) {
        $valor_exacto = sin($angulo['valor']);
        $valor_taylor = mi_sin($angulo['valor'], 'taylor', 10);
